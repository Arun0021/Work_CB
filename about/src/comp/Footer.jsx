...
// Removed: import { useTheme } from '../context/ThemeContext'; // No longer needed as isDarkMode is passed as a prop

// Footer now receives isDarkMode as a prop
const Footer = ({ isDarkMode }) => { // isDarkMode is now directly destructured from props
  // Removed: const { isDarkMode } = useTheme(); // This line is no longer needed

  // State for the mouse parallax effect on the "EVERYTHINGTALENT.AI" text
  ...
  useEffect(() => {
    const handleMouseMove = (event) => {
      ...
      // Invert the movement for a more natural "looking into" effect
      const diffX = event.clientX - centerX;
      const diffY = event.clientY - centerY;

      // Apply a small multiplier for the parallax effect
      // Adjust these multipliers (e.g., 0.005) to control the movement intensity
      setTextOffset({
        x: -diffX * 0.005,
        y: -diffY * 0.005,
      });
  ...
  // Keeping black for light mode as per original, but making it responsive to dark mode
  const footerBgClass = isDarkMode ? 'bg-gray-950' : 'bg-black';
  const footerTextColorClass = isDarkMode ? 'text-gray-300' : 'text-gray-300'; // Text color remains gray-300

  const linkTextColor = isDarkMode ? 'text-gray-400' : 'text-gray-400'; // Links remain gray-400
  const linkHoverColor = isDarkMode ? 'hover:text-white' : 'hover:text-white'; // Hover remains white

  const headingTextColor = isDarkMode ? 'text-white' : 'text-white'; // Headings remain white

  const skylineBgImage = `url(${bg.src})`; // Use .src for Next.js Image module


  return (
    <footer className={`relative ${footerBgClass} ${footerTextColorClass} py-12 overflow-hidden`}>
      ...
      {/* Responsive grid definition:
          ...
          - lg: 4 columns
      */}
      <div className="container mx-auto px-4 md:px-8 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 pb-12">
        {/* Left Section: Logo, Description, Social Icons */}
        {/* Adjusted col-span for responsiveness: full width on xs, 2 columns on sm, 3 on md, 1 on lg */}
        <div className="col-span-1 sm:col-span-2 md:col-span-3 lg:col-span-1">
          <div className="flex items-center mb-4">
            {/* Logo image, h-10 is a fixed height, w-auto maintains aspect ratio */}
      ...
      <div
        className="relative w-full h-48 sm:h-64 mt-8"
        style={{
      ...
      >
        ...
        <div
          className="absolute bottom-4 right-4 text-white text-2xl sm:text-3xl md:text-4xl font-extrabold tracking-wider z-10"
          style={{
