...
const Part2 = ({ isDarkMode }) => {
  ...
  const commonDescriptionTextClasses = "text-base mb-6"; // This text color will be dynamic
  ...
  const stickyCardBgBorderClass = isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200';
  const titleTextColor = isDarkMode ? 'text-purple-400' : 'text-purple-600';
  const yearTextColor = isDarkMode ? 'text-white' : 'text-gray-900';
  const descriptionTextColor = isDarkMode ? 'text-gray-300' : 'text-gray-700';
  const statCardBgClass = isDarkMode ? 'bg-gray-700' : 'bg-gray-100';
  const statCardTextColor = isDarkMode ? 'text-white' : 'text-gray-900';


  return (
    <div
      className={`${commonContainerClasses} ${containerBgClass} ${containerTextColor} py-16`}
    ...
    >
      {fullDisplaySequence.map((card, index) => (
        <div
          key={`${card.id}-${index}`}
          className={`${commonStickyCardWrapperClasses} ${fixedCardHeight} ${stickyCardBgBorderClass}`}
        ...
        >
          <div className={`${commonIndividualCardContentClasses}`}>
            ...
            <div className={`${commonTextContentClasses} md:w-1/2`}>
              <div>
                <p className={`text-sm font-semibold uppercase ${titleTextColor} mb-2`}>
                  {card.title}
                </p>
                <h3 className={`${commonYearTextClasses} ${yearTextColor}`}>
                  {card.year}
                </h3>
                <p className={`${commonDescriptionTextClasses} ${descriptionTextColor}`}>
                  {card.description}
                </p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 mt-auto">
                {card.stats.map((stat, statIndex) => (
                  <div key={statIndex} className={`${commonStatCardClasses} ${statCardBgClass} ${statCardTextColor}`}>
            ...
            <div className={`${commonImageContainerClasses} md:w-1/2`}>
              ...
              <Image
                ...
                width={500} // Example width, adjust as needed or use fill
